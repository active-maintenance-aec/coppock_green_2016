...
Two further facts about the deposit are worth recording. Running it writes nine files into its working directory---four `.tex` tables, four household tables and an `Rplots.pdf`---which is why it is never run inside `original/`. And four of its tables are built as `xtable` objects whose print calls are commented out, so those scripts compute Table 6 and Tables A6 and A7 and display nothing at all.
